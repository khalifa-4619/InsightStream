              ...
              {!selectedDs ? (
                ...
                <div className="bg-slate-950 rounded-3xl border border-slate-800 overflow-hidden min-h-[70vh] flex flex-col">
                  ...
                  <div className="flex border-b border-slate-800 bg-slate-950/50">
                    <button onClick={() => setActiveTab('laundry')} className={`flex-1 flex items-center justify-center gap-2 py-4 text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'laundry' ? 'text-indigo-400 border-b-2 border-indigo-500 bg-indigo-500/5' : 'text-slate-500 hover:text-slate-300'}`}>
                      <Wand2 size={16} /> Data Laundry
                    </button>
                    <button onClick={() => setActiveTab('eda')} className={`flex-1 flex items-center justify-center gap-2 py-4 text-xs font-bold uppercase tracking-wider transition-all ${activeTab === 'eda' ? 'text-indigo-400 border-b-2 border-indigo-500 bg-indigo-500/5' : 'text-slate-500 hover:text-slate-300'}`}>
                      <BarChart3 size={16} /> EDA Engine
                    </button>
                    <button className="flex-1 flex items-center justify-center gap-2 py-4 text-xs font-bold uppercase tracking-wider text-slate-700 cursor-not-allowed group relative">
                    ...
                    {activeTab === 'laundry' && (
                      <div className="space-y-8 animate-in fade-in duration-500">
                        <div className="flex justify-between items-start">
                          ...
                          <button onClick={() => runOperation('clean')} disabled={loading} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold flex items-center gap-2 transition-all disabled:opacity-50">
                      ...
                      <div className="space-y-6 animate-in fade-in duration-500" id="eda-report-area">
                        <div className="flex justify-between items-end">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${analysisResult ? 'bg-indigo-600 text-white' : 'border border-slate-700 text-slate-300'}`}
                            >
                            ...
                            <button
                              ...
                              className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${correlationData ? 'bg-indigo-600 text-white' : 'border border-slate-700 text-slate-300'}`}
                            >
                        ...
                        {loading && !analysisResult && !correlationData ? (
                          ...
                          <div className="space-y-6">
                            ...
                            {recommendations.length > 0 && !recommendationsApplied && (
                              <div className="flex justify-end">
                                <button
                                  onClick={() => runOperation("apply_recommendations", { recommendations })}
                                  disabled={loading}
                                    ...
                                    <div className="absolute top-4 right-4 flex gap-1.5 z-10">
                                      {colData.insights.map((ins, index) => (
                                        <div
                                          key={index}
                                          title={ins.text}
                                          className={`w-2.5 h-2.5 rounded-full cursor-help ${ins.type === 'critical' ? 'bg-red-500 animate-pulse' : ins.type === 'warning' ? 'bg-amber-500' : 'bg-indigo-400'}`}
                                        />
                                      ))}
                                    </div>
                                  )}
                          ...
                          <EmptyState
                            ...
                            description="Click 'Distributions' or 'Correlations' to process the logs."
                          />
                        ) : null}

                        {/* Correlation Matrix (safe check added) */}
                          ...
                          <div className="mt-0 space-y-4 animate-in slide-in-from-bottom duration-700">
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 overflow-x-auto">
                              <div
                                ...
                                style={{ gridTemplateColumns: `80px repeat(${correlationData?.columns?.length}, minmax(60px, 1fr))` }}
                              >
